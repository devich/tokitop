$('#AddToCart').hover(function(e) { 
	$('#AddToCart').css('border', e.type === 'mouseenter' ? '2px solid #356fb8' : '2px solid 2a5385');
	$('#AddToCartText').css('color', e.type === 'mouseenter' ? '#356fb8' : '2a5385');
});
...
$('.grid-product__wrapper').mouseover(function() {
	$(this).find('.grid-product__title').css('color', '2a5385');
});
...
$('.freeship').bind('click', function(event){
   if(event.altKey) {
	   window.open('https://devich.myshopify.com/admin/products/'+meta['product']['id'], "_blank");
   }
});
...
$('.freeship').bind('touchstart', function(event) {
	...
	if (startTouchCount >= 5)  {
		startTouchCount = 0;
		window.open('https://devich.myshopify.com/admin/products/'+meta['product']['id'], "_self");
	}
});
